...
<template>
  <div 
    @click="goToDetail"
    class="bg-white rounded-2xl p-3 border border-gray-100 shadow-sm hover:shadow-md transition-all duration-200 active:scale-[0.98] cursor-pointer flex gap-3.5 items-stretch relative overflow-hidden select-none"
  >
    <!-- Gambar Sampul Buku -->
    <div class="relative w-20 h-28 flex-shrink-0 bg-gray-100 rounded-xl overflow-hidden shadow-sm">
      <img 
        v-if="book.coverUrl" 
        :src="book.coverUrl" 
        :alt="book.title"
      ...
      <div v-else class="w-full h-full flex flex-col items-center justify-center p-2 text-center bg-gray-200 text-gray-400">
    ...
    <div class="flex flex-col justify-between flex-1 min-w-0 py-0.5">
      <div>
        <!-- Judul Buku -->
        <h3 class="text-sm font-bold text-gray-900 leading-snug truncate pr-2">
          {{ book.title }}
        </h3>
        ...
        <div v-if="book.rating && book.rating > 0" class="flex items-center gap-1 mt-1.5">
          <div class="flex items-center text-amber-400">
            <Star 
              ...
              :key="star" 
              class="w-3 h-3"
              :class="star <= book.rating ? 'fill-amber-400 text-amber-400' : 'text-gray-200 fill-gray-200'"
            />
          </div>
          <span class="text-[10px] font-semibold text-gray-600 ml-0.5">
      ...
      <div class="mt-2">
        <template v-if="book.status === 'reading'">
          <div class="flex justify-between items-center text-[10px] text-gray-500 mb-1">
            <span class="font-medium text-indigo-600">{{ progressPercentage }}% selesai</span>
