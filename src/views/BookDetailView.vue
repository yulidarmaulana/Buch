...
<template>
  <div class="min-h-screen bg-gray-50 pb-24 max-w-md mx-auto">
    ...
    <div v-else-if="!book" class="p-8 text-center">
      ...
      <h2 class="text-sm font-bold text-gray-900">Buku Tidak Ditemukan</h2>
      <p class="text-xs text-gray-500 mt-1 mb-4">
        Data buku mungkin telah dihapus atau tautan tidak valid.
      </p>
      <button 
        @click="goBack" 
        class="px-4 py-2 bg-gray-900 text-white rounded-xl text-xs font-semibold"
      >
        Kembali ke Rak Buku
      </button>
    </div>
    ...
    <template v-else>
      <!-- Navigasi Atas -->
      <div class="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-gray-100 px-4 py-3 flex items-center justify-between">
        <button 
          @click="goBack" 
          class="p-2 -ml-2 rounded-full text-gray-600 hover:bg-gray-100 transition-colors"
        >
          <ArrowLeft class="w-5 h-5" />
        </button>
        <span class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Detail Buku</span>
        <button 
          @click="handleShare" 
      ...
      <div class="relative overflow-hidden bg-gray-900 pt-8 pb-10 px-4 flex flex-col items-center">
        <div 
          v-if="book.coverUrl"
      ...
      <div class="px-4 -mt-4 relative z-20 space-y-4">
        ...
        <!-- Quick Stats -->
        <div class="bg-white rounded-2xl p-4 border border-gray-100 shadow-xs flex items-center justify-around text-center">
          <!-- Rating -->
          <div>
            <span class="text-[10px] text-gray-400 block mb-0.5">Rating</span>
          ...
          <!-- Total Halaman -->
          <div>
            <span class="text-[10px] text-gray-400 block mb-0.5">Halaman</span>
            <span class="text-sm font-bold text-gray-900">{{ book.totalPages || 0 }} hal</span>
          </div>
          ...
          <div>
            ...
            <span 
              ...
              :class="{
                'bg-indigo-50 text-indigo-700': book.status === 'reading',
                'bg-green-50 text-green-700': book.status === 'completed',
                'bg-rose-50 text-rose-700': book.status === 'wishlist'
              }"
            >
        ...
        <div v-if="book.status === 'reading'" class="bg-white rounded-2xl p-4 border border-gray-100 shadow-xs">
          <div class="flex justify-between items-center text-xs text-gray-600 mb-2">
            <span class="font-semibold text-indigo-600">Progres Membaca</span>
        ...
        <div class="bg-white rounded-2xl p-4 border border-gray-100 shadow-xs">
          <div class="flex items-center gap-2 mb-3">
            <Quote class="w-4 h-4 text-indigo-600" />
            <h2 class="text-sm font-bold text-gray-900">Catatan & Ulasan Pribadi</h2>
          </div>

          <div v-if="book.review" class="text-xs text-gray-700 leading-relaxed bg-gray-50 p-3.5 rounded-xl border border-gray-100 whitespace-pre-line">
            "{{ book.review }}"
          </div>
          ...
          <div v-if="book.startedAt || book.finishedAt" class="mt-4 pt-3 border-t border-gray-100 flex items-center justify-between text-[11px] text-gray-400">
